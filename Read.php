...
$SN = $_GET['key'];
// include("module/SendMessage/SendMessage.php");
// Smg_Check($SN);
if(strlen($SN)<=12 && strlen($SN)>0){
	...
	$conn->close();
	$json_data = new stdClass(); //初始化json数据
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			$Security = $row["Security"]; //安全等级 0公开 1私密 2半公开隐藏NOTE,N1,N2
			$json_data->Security = $Security;				
			switch ($Security){
				case 0:
					$json_data->SN = $SN = $row["SN"];
					$json_data->TAG = $TAG = $row["TAG"];
					$json_data->TIME = $TIME = $row["TIME"];
					$json_data->NOTE = $NOTE = $row["NOTE"];
					$json_data->N1 = $N1 = $row["N1"];
					$json_data->N2 = $N2= $row["N2"];
					break;
				case 1:
				...
				case 2:
					...
					$json_data->TAG = $TAG = $row["TAG"];
					$json_data->TIME = $TIME = $row["TIME"];
					$json_data->N2 = $N2= $row["N2"]; //N2属于半公开
					if (!empty ( $_SESSION ['user'] )){
						$json_data->NOTE = $NOTE = $row["NOTE"];
						$json_data->N1 = $N1 = $row["N1"];
					}
